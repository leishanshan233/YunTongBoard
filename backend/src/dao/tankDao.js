...
const findAll = async () => {
  const [rows] = await pool.query(`
    SELECT
      t.id, t.tank_code, t.tank_name, t.row_index, t.col_index,
...
const findEmpty = async () => {
  const [rows] = await pool.execute(
    `SELECT * FROM tanks WHERE status = ? ORDER BY row_index ASC, col_index ASC`,
    ['idle']
  );
  return rows;
};
...
const findByCode = async (tankCode) => {
  const [rows] = await pool.execute('SELECT * FROM tanks WHERE tank_code = ?', [tankCode]);
  return rows[0] || null;
};
...
const create = async (conn, data) => {
  ...
  const [result] = await db.execute(
    'INSERT INTO tanks (tank_code, tank_name, row_index, col_index, status, created_user_id) VALUES (?, ?, ?, ?, ?, ?)',
    [data.tank_code, data.tank_name || null, data.row_index, data.col_index, 'idle', data.created_user_id || null]
  );
...
const update = async (conn, id, data) => {
  const db = conn || pool;
  await db.execute(
    'UPDATE tanks SET tank_code = ?, tank_name = ?, row_index = ?, col_index = ?, modified_at = NOW(), modified_user_id = ? WHERE id = ?',
    [
      data.tank_code,
      data.tank_name ?? null,
      data.row_index,
...
const count = async () => {
  const [rows] = await pool.query(`SELECT COUNT(*) AS count FROM tanks`);
  return rows[0].count;
};
